...
var buyContractInputs = [
                            ...
                            { label: 'Bank Contract Amount', api: 'Bank_Contract_Amount__c', type: 'number', required: false },
                            { label: 'Spot Rate', api: 'Spot_Rate__c', type: 'number', step: "0.0000001", required: false},
                            { label: 'Premium(+) / Discount(-)', api: 'Premium_Discount__c', type: 'number', step: "0.0000001", required: false },
...
export default class TreasuryManagement_PO extends LightningElement {
    ...
    handleFilter(event)
    {
        ...
        let filteredRecs = this.originalRecs.filter(rec => {
            ...
            {
                if (rec[key] === undefined || rec[key] != this.filterChanges[key]) return false;
            }
