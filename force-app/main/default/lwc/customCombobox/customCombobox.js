...
import {evaluateExpression,syncComboboxAttributes,handleSelectionRemovalAction,clearSelectionOnAutocomplete,handleKeyUpOnInput,handleOptionClickAction,handleOptionHoverAction,handleDropdownOnInputClick,handleKeyDownOnInput,openAndCloseDropdown,preventAndStopEvent} from './customComboboxHelper';
const SEPERATOR = ';',
      MAX_OPTION_DISPLAY = 3,
      VALIDATION = [{name:"REQUIRED",condition:"(value === null || value === undefined || (typeof value === 'string' && value.trim() === '')) ? true : false", message: "Please select", custom:false}];
...
@track comboboxObj = {
    ...
    get _clearMode(){
        return this.autocomplete && (this.value !== '' && this.value && this.value.length > 0 && !this.multiselect) ? true : false;
    },
...
@api setCustomValidity(name, message,condition = 'true'){
    this.setCustomErrorMessage(name, message,condition);
}
...
handleSelectionRemoval(event){
    ...
    setCustomErrorMessage(name,msg,cond){
        let dataObj = {name: name || "_custom",condition:cond,message:msg,custom:true};
    ...
    @api refreshSelectedValue()
    {
        this.comboboxObj.value = '',
        this.comboboxObj._valueDisplay = '';
        this.comboboxObj.hasError = false;
    }
}