...
var addnlContractInputs =   [
                                ...
                                { label: 'Bank Contract No (Sell/Execute)', api: 'Bank_Contract_No_Sell_Execute__c', type: 'text', value: null, required: false },
                                { label: 'Effective Date (Sell/Execute)', api: 'Effective_Date_Sell_Execute__c', type: 'date', value: null, required: false },
                                { label: 'Transaction Date (Sell/Execute)', api: 'Transaction_Date_Sell_Execute__c', type: 'date', value: date, required: false },
                                { label: 'Spot Rate (Sell/Execute)', api: 'Spot_Rate_Sell_Execute__c', type: 'text', isNum: true, value: null, required: false },
                                { label: 'Premium(+) / Discount(-)', api: 'Premium_Discount__c', type: 'text', isNum: true, value: null, required: false },
                            ];
...
export default class TreasuryManagement_FC extends LightningElement {
    ...
    validateSubmit()
    {
        return this.validateData() && this.validateInput();
    }

    validateData()
    {
        let errMsg = '';
        let contractStatus = this.changes['Status__c'];

        this.selectedLineItems.forEach(item => {
            if (contractStatus == 'Forward Contract Execute')
            {
                if (!item.ETA_Date__c)
                {
                    errMsg = 'Items must have ETA Date for Execute Contract. Please check again!';

                    this.toast('WARNING!', errMsg, null, 'warning', 'sticky');

                    return false;
                }
                if (item.Final_Vehicle_Price__c == 0)
                {
                    errMsg = 'Items must have Final Vehicle Price for Execute Contract. Please check again!';
            ...
            {
                errMsg = 'Items must have SCL Vehicle Status for PO Cancellation Contract. Please check again!';

                this.toast('WARNING!', errMsg, null, 'warning', 'dismissible');

                return false;
            }
            else if (contractStatus == 'Forward Contract Duty Sale' && !item.Duty_Payment_Date__c)
            {
                errMsg = 'Items must have Duty Payment Date for Duty Sale Contract. Please check again!';
    ...
    validateInput()
    {
        ...
        {
            let inputEl = this.addnlContractInputs.find(input => input.api == key);
            ...
            if (inputEl.api == 'Spot_Rate_Sell_Execute__c' && Number(value) < 0)
            {
    ...
    handleSubmit()
    {
        if (this.changes && this.validateSubmit())
        {
            this.spinner = true;
